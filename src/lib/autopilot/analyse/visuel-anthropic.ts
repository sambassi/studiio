...
export function fournisseurAnthropic(transport?: Transport): FournisseurVisuel | null {
  ...
  return async (entree) => {
    ...
    contenu.push({
      type: 'text',
      text: 'Réponds uniquement par le JSON demandé : '
        + '{"resume":string,"textesVisibles":[{"texte":string,"seconde":number,'
        + '"confiance":number}],"qualite":{"scoreGlobal":number,"nettete":number,'
        + '"lumiere":number,"cadrage":number,"energie":number,"interetVisuel":number,'
        + '"problemes":string[]}}',
    });
