 ...
 */
export type { CardData, ComposerOptions, DesignOptions, AudioKeyframe };
...
export interface CanonicalMedia extends OpenRecord {
   ...
   * MONTAGE pour les posts les plus anciens. `toComposerOptions` prefere
   * donc `rushUrls[0]`, comme le fait deja la regeneration du Calendrier.
   */
...
/** Voix par sequence. Cles FR — c'est la forme attendue par le compositeur. */
export type CanonicalSequenceVoiceUrls = NonNullable<ComposerOptions['sequenceVoiceUrls']> & OpenRecord;
 ...
 * persistee ; c'est `toComposerOptions` qui fait la traduction.
 */
...
/**
 * Ce que `toComposerOptions` peut produire a partir des seules metadonnees.
 *
 * `title` est une COLONNE du post, pas une metadonnee : le contrat ne peut
 * pas l'inventer. `width` / `height` ne sont connus que si `videoSize` a ete
 * enregistre. Les objets de rendu (`onProgress`, `sharedAudioCtx`,
 * `musicBuffer`, `voiceBuffer`) appartiennent a l'appelant, au moment du
 * rendu — le contrat n'en sait rien et ne doit rien en supposer.
 */
export type ComposerOptionsFromDesign = Omit<
  ComposerOptions,
  'width' | 'height' | 'title' | 'onProgress' | 'sharedAudioCtx' | 'musicBuffer' | 'voiceBuffer'
> & {
  width?: number;
  height?: number;
};
