 ...
 *
 * Asymetrie a connaitre : quand `originalMetadata` est FOURNI, c'est lui qui
 * fait foi pour les cles non gerees, et `design.passthrough` est ignore. Un
 * appelant qui aurait ajoute une cle inconnue dans `passthrough` ne la
 * verrait donc pas ressortir. Ce cas ne se produit jamais en aller-retour
 * (le passthrough vient de l'original), et `mergePostMetadata` le couvre
 * explicitement pour le fragment entrant du client.
 */
