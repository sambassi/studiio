...
describe('M3-B2 n ajoute AUCUNE migration', () => {
  ...
  /**
   * L'exception de M3-G suit exactement le même moule que M3-F : créer sa
   * table, référencer `rush_clip_sets`, et y poser l'index unique que sa clé
   * étrangère composite EXIGE. Rien d'autre — et surtout pas de retour sur
   * les tables déjà en production.
   */
  it('M3-G n ajoute qu un index à `rush_clip_sets`, sans rien détruire', () => {
    const fichier = '2026-09-05-rush-montage-plans.sql';
    expect(fichiersMigration).toContain(fichier);
    const code = sqlSansCommentaires(fichier).toLowerCase();

    expect(code).toMatch(/create table if not exists public\.rush_montage_plans/);

    // Le seul contact avec une table existante : l'index que la clé étrangère
    // à deux colonnes exige, et la référence elle-même.
    expect(code).toMatch(
    ...
    expect((code.match(/references public\.rush_clip_sets/g) ?? []).length).toBe(1);
    expect(code, 'aucun ALTER, sur quoi que ce soit').not.toMatch(/alter\s+table/);

    // Les analyses, les rushes, les candidats et les transcriptions sont hors
    // de son périmètre — comme la file de rendu et la table de facturation.
    expect(code).not.toContain('rush_analyses');
    expect(code).not.toContain('rush_transcriptions');
    expect(code).not.toContain('rush_candidate_sets');
    ...
    for (const interdit of [
      ...
      expect(code, `M3-G ne doit pas contenir « ${interdit} »`).not.toContain(interdit);
    }
    expect(code).not.toMatch(/\bgrant\b/);
  });
