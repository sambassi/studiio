...
describe('Brouillon — référence de LUT', () => {
  ...
  it('ramène une intensité hors plage à la pleine intensité', () => {
    ...
    expect(sanitizeDraft(valid({ lut: { ...LUT, intensity: 'fort' } }), DEPS)!.lut!.intensity).toBe(1);
  });
  ...
  it('borne un nom démesuré', () => {
    const d = sanitizeDraft(valid({ lut: { ...LUT, name: 'x'.repeat(500) } }), DEPS)!;
    expect(d.lut!.name).toHaveLength(120);
  });
});
